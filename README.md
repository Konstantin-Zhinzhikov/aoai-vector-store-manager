...
После ввода vector store ID скрипт отдельно спросит, нужно ли окончательно удалить Azure files через `DELETE /files/{file_id}`.

Удаление самих Azure files необратимо. Этот режим нужно явно подтвердить вводом `YES`, а перед выполнением дополнительно вводится `DELETE FILES`.
...
- Повторный запуск очистки безопасен для уже удаленных attachments, но подтвержденный режим удаления может окончательно удалить сами Azure files.
